...
class SessionStore:
    ...
    def apply_move(self, session_id: str, source: str, target: str) -> None:
        with self._lock:
            session = self._sessions.get(session_id)
            if session is None:
                raise KeyError(session_id)
